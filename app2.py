...
#updates the database to refesh contents to unlock account 
...
#updates database with te failed attempts number 
def failed_log (user_id, failed_attempts):
    try: 
        print (" values inside the fail log: user_id={user_id}, attempts = {failed_attempts} ") ##checking to see what values the function will hold 
...
def login():
    ...
    """
    #from datetime import datetime, timedelta #imports needed functions
    ip_address = request.remote_addr #fetches ip address of computer 
    MAX_login = 5  #max log variable 
    locked_out_minutes = 15 
        ...
        try:
            ...
            if user is None:
                ...
                if user ["locked_until"] and datetime.now() < user ["locked_until"]:
                    unlock_time=user ["locked until"].strftime("h:m")
                    error = "account locked due to too many failed attempts. Try again after {unlock_time}"
                    #log event here to user id 
                    ...
                    if bcrypt.checkpw(password_bytes, stored_hash_bytes): #checks password
                        # Create session after successful authentication
                        session["user_id"] = user["user_id"]
                        session["username"] = user["username"]
                        session["role"] = user["role"]
                        unlock_account(user["user_id"])
                        #refresh failed attempts if any
                        ...
                        #error searching 
                        print ("password block reached ")
                        print ("user_id: {user['user_id']}")
                        print ("failed_attempts from db: {user['failed_attempts']}")


                        new_attempts = user["failed_attempts"] + 1
                        print ("new_attempts: {new_attempts}")  #'checking value 
                        print ("calling failed log")   #checking to see if this fucntion is called 
                        failed_log(user["user_id"], new_attempts)
                        print (" failed log complere")   #checking its completion 
    
                        if  new_attempts >= MAX_login: #cheking if limit reached
                            #locked_until= datetime.now () + timedelta(minutes= locked_out_minutes)
                            lock_account(user["user_id"], locked_out_minutes) #calls lock function
                            error = "too manny faile attempts. Acount is locked for {locked_out_minutes} minutes "
                             #display contents?   
                            print ("max login attempts reached, please try again in 15 minutes")
                            
                        else: #calculates remaining 
                            remaining= MAX_login - new_attempts #
                            print ("incorect username and password. {remaining} attempts remaining")
                            error= "inncorrect credentials, {remaining} attempts left" #displays content
                            #log event 
